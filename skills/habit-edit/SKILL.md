...
3. **Skip this step if you are only asking the user a question.** Otherwise: read `~/.claude/skills/habit-shared/PROCESSING.md` and apply its rules for interpretation, deduplication, and scope detection. Edit the habit in the scope where it was found. To change scope, the user must explicitly request it.
